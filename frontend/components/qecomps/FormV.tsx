      ...
      props.items.map((item, i) => {
        ...
        if (i % 2 == 0) {
          return <f-cc key={"form_"+i+"_"+ item}>
            <f-cc  class={z.qestyles.hover} style={{ ...optstyle, backgroundColor: it.name == props.strvalue ? 'green' : btnbg }}
             onClick={() => { props.on?.(it.name, it.value) }}>
              <f-11><sp-2 />{Img}{(it.name).toLocaleString(z.lang.region)}<sp-2 /></f-11>
            </f-cc>
            {itp ? <f-cc class={z.qestyles.hover} style={{ ...optstyle, backgroundColor: itp.name == props.strvalue ? 'green' : btnbg }} 
            onClick={() => { props.on?.(itp.name,itp.value) }}>
              <f-11><sp-2 />{Img}{(itp.name).toLocaleString(z.lang.region)}<sp-2 /></f-11>
            </f-cc> : null}
          </f-cc>
        }
        return null;
