...
export default (props:{
  ...
  return <>
    <div className={z.qestyles.searchbox}>
      ...
      <div style={{ width: "100%", marginTop: 5, padding: "0.5rem 0.5rem 0.5rem 0.5rem", fontSize: 12, fontWeight: 600 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", marginTop: 3 }}>
          <div style={{ display: "flex", marginTop: -0.5, cursor: "pointer", height: 28, width: 40, position: "relative" }}>
            <div id="smenu" style={{ position: "absolute", width: 50, height: 80, top: -30, paddingTop: 30, opacity: isopen || iso ? 1 : 0.3, }}
              onClick={(e) => { toggle() }}>
              &nbsp;<img src={global.cdn("/files/menucat.svg")} alt={z.lang.addopts} style={{ width: 28, height: 28 }}
              />&nbsp;</div></div>
          <input id="search_input" style={{ textAlign: "left", paddingLeft: 5, marginTop: 0, }} className={z.qestyles.txt3} placeholder={props.placeholder}
        ...
        {(cat || sort) ? <div style={{ height: 8, display: "flex", marginTop: -4 }}>
          {cat ? <><Text value={z.lang.category + " " + Cap(cat)} islink fontSize={10} on={() => { toggle() }} />&nbsp;&nbsp;</> : null}
          {sort ? <Text value={z.lang.sort + " " + Cap(sort)} islink fontSize={10} on={() => { toggle() }} /> : null}
        </div> : null}
      ...
      <div id={"search_options_" + props.id} style={{ overflow: "hidden", maxHeight: "0px", paddingTop: 3 }}>
        <div className={z.qestyles.ft}>
          <span style={{ color: "#452700",fontSize: 12, fontWeight: 600 }}>{z.lang.catfilter}</span>
        </div>
        <div>
          <div className={z.qestyles.dbar} style={{ paddingTop: 5, paddingRight: 5, paddingLeft: 5, }}>
        ...
        <br-x />
        <div className={z.qestyles.ft}>
          <span style={{ color: "#452700", fontSize: 12, fontWeight: 600 }}>{z.lang.howtosort}</span>
        </div>
        <div className={z.qestyles.dbar} style={{ paddingTop: 5, paddingRight: 5, paddingLeft: 5, }}>
