  ...
  let z = SSRGlobal()
    return <>
          ...
          <div style={{marginTop:5, display:"flex"}}>
            <input id={props.id+"upload_inptxt"} style={{textAlign:"left", paddingLeft:10}} className={z.qestyles.txtfile} placeholder="" type='text'
            ...
            <f-cc class={z.qestyles.select_file} 
            style={{borderRadius:z.lang.dir == "ltr"?"0 8px 8px 0":"8px 0 0 8px"}}
            onClick={()=>{uploaders[props.id]?.open?.()}}>{z.lang.selectfiles}</f-cc>
          </div>

          <br-x/>
            {props.explains?<span style={{fontSize:10}}>.jpg, .jpeg, .png, .txt, .pdf, .zip, .rar , .js, .cs, .dll, .py <br/>{z.lang.max5f10mb}</span>:null}
