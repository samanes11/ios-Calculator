...
export default (props) => {
    ...
    return <WindowFloat title={z.lang.choose} style={{ maxHeight: 400, maxWidth: 400 }} onclose={() => { props.onclose?.() }}>
        <TextBox placeholder={z.lang.search} deltxt on={(txt) => {
            if (txt == "") {
                        ...
                        on={async () => {
                            if (state.selectedsyms[s.name]) {
                                if (props.asktoremove) {
                                    if (await confirmer(z.lang.delete, z.lang.delconf)) {
