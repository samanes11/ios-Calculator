...
})=>
{
  ...
  if(props.items.length > len)
  {
    ...
    lastrow[lastrow.length - 1] = <VItem key="more_vitem" title={z.lang.more} image={global.cdn("/files/more.svg")} s={45} 
    on={()=>{setForm("more")}} txtmt={props.txtmt}/>
  }

  return <>

  {form=="more"?<WindowFloat title={z.lang.selcurrency} onclose={()=>setForm(null)}>
    {
  ...
  <Window title={props.title}>  
        {packs.map((row,i)=>{
          return <div key={"vitem_r"+ i} className={z.qestyles.dbar}>
            {row}
          </div>
        })}
  </Window></>
